...
(function () {
  ...
  function lfdInfo(r){
    let cfg={terminalFreeDays:3,detentionFreeDays:14,warningDays:4,criticalDays:2,demRate20:100,demRate40:225};
    try { cfg=Object.assign(cfg,JSON.parse(localStorage.getItem('gml_phase1_config_v2')||'{}')); } catch(e){}
    const portIn=parseDate(getField(r,['PORT IN']));
    const portOut=parseDate(getField(r,['PORT OUT']));
    const returned=parseDate(getField(r,['CONTAINER RETURN DATE','EMPTY RETURN DATE']));
    const terminalDays=Number(cfg.terminalFreeDays||0);
    ...
    if(portIn){
      portLfd=new Date(portIn); portLfd.setDate(portLfd.getDate()+terminalDays);
      if(portOut){
        const dwell=Math.max(0,Math.floor((portOut-portIn)/86400000));
        demDays=Math.max(0,dwell-terminalDays); cost=demDays*rate; portState='complete';
        portDaysLeft=Math.floor((portLfd-portOut)/86400000);
      }else{
        portDaysLeft=Math.floor((portLfd-today())/86400000);
        portState=portDaysLeft<0?'overdue':portDaysLeft<=Number(cfg.criticalDays)?'critical':portDaysLeft<=Number(cfg.warningDays)?'warning':'safe';
      }
    }
  ...
  function candidates(){
    ...
    (rows||[]).forEach((r,i)=>{
      ...
      if(!l.returned){
        if(l.demDays>0) out.push({i,container,exceptionKey:'DEM',title:'Port Demurrage exposure',severity:'CRITICAL',detail:l.demDays+' demurrage day(s) · 
...
+Math.round(l.cost)});
        else if(l.portState==='overdue') out.push({i,container,exceptionKey:'LFD_PORT',title:'Port LFD expired',severity:'CRITICAL',detail:Math.abs(l.portDaysLeft)+' day(s) overdue'});
        else if(l.portState==='critical') out.push({i,container,exceptionKey:'LFD_PORT',title:'Port LFD critical',severity:'HIGH',detail:l.portDaysLeft===0?'Port LFD today':l.portDaysLeft+' day(s) left'});
        else if(l.portState==='warning') out.push({i,container,exceptionKey:'LFD_PORT',title:'Port LFD warning',severity:'MEDIUM',detail:l.portDaysLeft+' day(s) left'});
        if(l.outsideState==='overdue') out.push({i,container,exceptionKey:'LFD_OUTSIDE',title:'Outside Port LFD expired',severity:'CRITICAL',detail:Math.abs(l.outsideDaysLeft)+' day(s) overdue'});
        else if(l.outsideState==='critical') out.push({i,container,exceptionKey:'LFD_OUTSIDE',title:'Outside Port LFD critical',severity:'HIGH',detail:l.outsideDaysLeft===0?'Outside Port LFD today':l.outsideDaysLeft+' day(s) left'});
        else if(l.outsideState==='warning') out.push({i,container,exceptionKey:'LFD_OUTSIDE',title:'Outside Port LFD warning',severity:'MEDIUM',detail:l.outsideDaysLeft+' day(s) left'});
      }
      if(eta && eta<=today() && !portIn) out.push({i,container,exceptionKey:'PORT',title:'Port In pending',severity:'HIGH',detail:'ETA passed without Port In'});
      if(portIn && !cfs) out.push({i,container,exceptionKey:'CFS',title:'CFS In pending',severity:'MEDIUM',detail:'Container has Port In but no CFS In'});
      if(cfs && !dest) out.push({i,container,exceptionKey:'DESTUFF',title:'Destuffing pending',severity:'MEDIUM',detail:'CFS In completed'});
